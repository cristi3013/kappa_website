...
const BryantSection2 = () => {
  ...
  useEffect(() => {
    if (video1_view) {
      video1_ref.current.play();
    } else {
      video1_ref.current.pause();
      video1_ref.current.currentTime = 0;
    }
    if (video2_view) {
      video2_ref.current.play();
    } else {
      video2_ref.current.pause();
      video2_ref.current.currentTime = 0;
    }
    if (video3_view) {
      video3_ref.current.play();
    } else {
      video3_ref.current.pause();
      video3_ref.current.currentTime = 0;
    }
  ...
  return (
    <BryantSectionWrapperCss>
      <InfoBlockCss>
        <p>
          Alongside the product renders, smooth transitions and custom
          interactions ensure a pleasent experience.
        </p>
      </InfoBlockCss>
