...
const TitleLink = (props) => {
  ...
  return (
   <TitleLinkCss >
    <h3 onMouseOver={()=>setShow(true)} onMouseOut={()=>setShow(false)} onTouchStart={()=>setTouch(true)} onTouchCancel={()=>setTouch(false)}  onTouchMove={()=>setTouch(false)} onTouchEnd={()=>touched&&setShow(true)}>{props.children}</h3>
    {props.img&&(
      <>
